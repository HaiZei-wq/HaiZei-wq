/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2018年07月23日 星期一 15时15分46秒
 ************************************************************************/
...
typedef struct HFNode {
    char ch;
    int freq;
    struct HFNode *lchind, *rchild;
} HFNode;

HFNode *getNode() {
    HFNode *p = (HFNode *)malloc(sizeof(HFNode));
    p->rchild = p->lchind = NULL;
    p->freq = 0;
    return p;
}
...
void extract(HFNode *root, char *buff, char (*huffman_code)[100], int n) {
    ...
    buff[n] = '0';
    extract(root->lchind, buff, huffman_code, n + 1);
    buff[n] = '1';
